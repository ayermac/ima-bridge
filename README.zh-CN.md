...
- **WebView 扫码登录** — 内嵌 WebView 自动探测 localStorage 凭证，轮询页面信号（扫码成功、授权确认、二维码过期），支持手动登录回退
...
1. 点击 **"扫码登录"** — 内嵌 WebView 打开 IMA 登录页，自动检测扫码成功和凭证存储；若自动检测失败，可使用手动登录按钮
2. 浏览知识库，点击进入
3. 选择文件：
   - **下载** — 保存到本地目录
